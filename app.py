import time
import threading 
from threading import*


data = {} #dictionry to store the data


#Create Operation
def create(key,value,timeout=0):
    if key in data:
        print("error: this key already exists") #error message1
    else:
        if(key.isalpha()):
            if len(data)<(1024*1020*1024) and value<=(16*1024*1024): #constraints for file size less than 1GB and Jasonobject value less than 16KB 
                if timeout==0:
                    l=[value,timeout]
                else:
                    l=[value,time.time()+timeout]
                if len(key)<=32: #constraints for input key_name capped at 32chars
                    data[key]=l
            else:
                print("error: Memory limit exceeded!! ")#error message2
        else:
            print("error: Invalind key_name!! key_name must contain only alphabets and no special characters or numbers")#error message3


#Read Operation
            
def read(key):
    if key not in data:
        print("error: given key does not exist in database. Please enter a valid key") #error message4
    else:
        read =data[key]
        if read[1]!=0:
            if time.time()<read[1]: #comparing the present time with expiry time
                stri=str(key)+":"+str(read[0]) #to return the value in the format of JasonObject i.e.,"key_name:value"
                return stri
            else:
                print("error: time-to-live of",key,"has expired") #error message5
        else:
            stri=str(key)+":"+str(read[0])
            return stri


#Delete Operation

def delete(key):
    if key not in data:
        print("error: given key does not exist in database. Please enter a valid key") #error message4
    else:
        delete = data[key]
        if delete[1]!=0:
            if time.time() < delete[1]: #comparing the current time with expiry time
                del delete[key]
                print("key is successfully deleted")
            else:
                print("error: time-to-live of",key,"has expired") #error message5
        else:
            del data[key]
            print("key is successfully deleted")
